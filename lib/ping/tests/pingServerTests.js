...
describe('PingServer', function () {
  ...
  it('uses the cache on subsequent attempts', async () => {
    ...
    const dnsDelay = json.dnsDelay;
    const connectionDelay = json.connectionDelay;
    ...
    assert.equal(dnsDelay, json2.dnsDelay);
    assert.equal(connectionDelay, json2.connectionDelay);
  });

  it('it does not return a cached value if ttl has past', async () => {
    const requestOptions = {
      host: HOST,
      port: PORT,
      method: 'POST',
