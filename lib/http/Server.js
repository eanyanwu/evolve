...
/**
 * Creates an Http server and attaches handlers for edge cases so you don't have to
 */
function Server(
  { host = 'localhost', port = 80, name = 'Process', onListening },
  requestHandler = () => {}
) {
  assert.equal(typeof requestHandler, 'function');
  const server = createSimpleHTTPServer({ name, onListening }, requestHandler);
